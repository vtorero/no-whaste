...
$app->post("/reporte",function() use($db,$app){
    ...
    while ($row = $factura->fetch_array()) {
     ...
     $app->post("/predecir-producto",function() use($db,$app){
        header("Content-type: application/json; charset=utf-8");
        $json = $app->request->getBody();
        $dat = json_decode($json, true);
        $arraymeses=array('Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec');
        ...
        $venta_real=array();
        $sql="SELECT sum(cantidad*precio)merma,DATE_FORMAT(fecha_registro, '%Y-%m') fecha FROM venta_detalle where id_producto=".$dat['producto']." and  fecha_registro between  '".$ini."' and '".$fin."'   group by 2";
        $datos=$db->query($sql);
        $info=array();
        $dias=array();
        $dia=1;
    ...
    {
     $data=array();
        for($i=$date_ini;$i<=$date_end;$i = date("Y-m-d", strtotime($i ."+ 1 month"))){
